		...
		if ( matcher.find() ) {
			...
			switch ( matcher.group( 1 ) ) {
				...
				case "!request": {
					if (matcherArgs.isEmpty()) {
					...
					else if (matcherArgs.get(0).toLowerCase().matches("--slot=1 --random") && !matcherArgs.get(1).isEmpty()) {
						...
						if (contains) {
							event.respondWith("The timezone is valid, proceeding...");
							}
							else {
							event.respondWith("You have specified an invalid timezone.");
							return;
						}
